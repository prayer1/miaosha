...
class Goods Extends \Mysql\Crud {
     ...
     * @param int $status 状态 -1不限制状态
     * @return mixed
     */
    public function getListByActive($active_id = 0, $status = -1) {
        $params = array('active_id' => $active_id);
        if ($status < 0) {
            $sql = "SELECT * FROM `" . $this->table . "` WHERE active_id=:active_id ORDER BY `" . $this->pk . "` DESC";
        } else {
            $sql = "SELECT *FROM `" . $this->table . "` WHERE active_id=:active_id AND sys_status=:sys_status ORDER BY `" . $this->pk . "` DESC";
